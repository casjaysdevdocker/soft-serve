...
HOSTNAME="${HOSTNAME:-casjaysdev-soft-serve}"
...
SERVICE_PORT="23231"
SERVICE_NAME="soft"
SERVICE_COMMAND="$SERVICE_NAME serve"
export SOFT_SERVE_REPO_PATH="${SOFT_SERVE_REPO_PATH:-/data/soft/repos}"
export SOFT_SERVE_KEY_PATH="${SOFT_SERVE_KEY_PATH:-/config/ssh/soft_serve_server_ed25519}"
export SOFT_SERVE_INITIAL_ADMIN_KEY="${SOFT_SERVE_INITIAL_ADMIN_KEY:-}"
SOFT_SERVE_KEY_DIR="$(dirname "$SOFT_SERVE_KEY_PATH")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Show start message #
...
[ -d "/config/soft" ] || mkdir -p "/config/soft"
[ -d "$SOFT_SERVE_KEY_DIR" ] || mkdir -p "$SOFT_SERVE_KEY_DIR"
[ -d "$SOFT_SERVE_REPO_PATH" ] || mkdir -p "$SOFT_SERVE_REPO_PATH"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# begin main app
case "$1" in
healthcheck)
